...
const runtimeIDs = {
  active: [],
  new() {
    const next = runtimeIDs.active.length;
    runtimeIDs.active[next] = next + 1;
    return next;

  },
  done(id) {
    runtimeIDs.active.splice(id, 1);
  },
};
 ...
 */
function appendErrors(runtimeErrors, errors) {
  console.log('currently %d errors, appending %d more', runtimeErrors.length, errors.length);
  // errors.forEach(error => runtimeErrors.push(error));
  return runtimeErrors.concat(errors);
}
...
function cleanup(runtime, id) {
  // console.log('cleaning up', id, runtime.errors[id].length);
...
function createFlow(runner, tasks, options) {
  ...
  const flowRuntime = { opts, errors: new Map() };
  ...
  const flow = {
    ...
    run(providedContext, mainRuntime) {
      ...
      // } else {
      //   runtime.id = runtimeID = runtimeIDs.new();
      //   runtime.errors[runtimeID] = [];
      // }

      const runtimeID = runtime.errors.size;
      if (!mainRuntime) {
        runtime.errors.set(runtimeID, [])
      }

      console.log('flow %s starting', opts.name);
      console.log('with runtime: %s - %d', runtime.opts.name, runtimeID);
      ...
      return runner(flowTasks, runtime, opts)
        .then(data => {
          ...
          const errors = appendErrors(runtime.errors.get(runtimeID), data.errors);
          runtime.errors.get(runtimeID, errors);
          ...
          if (!opts.abortOnError) {
            flowResult.errors = errors.slice();
          }
        ...
        .then(data => {
          if (pipedFlows.length > 0) {
            ...
            return createFlow(Runner.waterfall, prepareTasks(pipedFlows, runtime), pipedFlowsOpts)
              ...
              .tap(() => cleanup(runtime, runtimeID));
          }

          cleanup(runtime, runtimeID);
