 ...
 * @param {*}    ctx            The running Flow's context
 ...
 */
function runTask(task, ctx, previousResult) {
  return new Promise((resolve, reject) => {
    ...
    const result = task.handler.apply(task.handler, callArgs);
...
module.exports = {
  ...
  create(id, handler/* , ...args */) {
    ...
    const task = {
       ...
       *
       * @param {Object} ctx   The running Flow's context
      ...
      run(ctx, value) {
        return stack.reduce((promise, task) => {
          return promise.then(previousResult => runTask(task, ctx, previousResult));
